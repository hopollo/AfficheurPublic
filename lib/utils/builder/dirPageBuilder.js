...
function DirPageBuilder(view) {
  fs.readdir(path.join(appRoot, 'public'), (err, folders) => {
    ...
    if (folders.includes(view))
      return UpdatePageContent(view);
  });
}

function UpdatePageContent(sourceFolder) {
  const source = path.join('public', sourceFolder);
  const target = path.join('views', sourceFolder + '.html');
  const skeletonPage = path.join(appRoot, 'lib', 'defaultPageSkeletton.html');
  
  GetDirContent(source);

  function GetDirContent(source) {
    fs.readdir(source, (err, files) => {
      if (err) return record(`ERROR (Builder): ${err}`);
